...
while read i; do
        ...
        curl -s -k -H "Content-Type: application/json" -X POST -d'{"shared-secret":"'$cp_gw_ia_pass'","ip-address":"'$i'","machine":"TORExitNode","identity-source":"TOR Exit Node Script","domain":"Production Domain","calculate-roles":1,"session-timeout":86400,"fetch-machine-groups":0,"roles":["'$cp_gw_role'"]}' $cp_gw_ia_url/add-identity | ./jq-linux64        
